...
class TestAudioDevice:
    ...
    def test_audio_device_creation(self):
        ...
        device = AudioDevice(
            id="{0.0.0.00000000}.{abc-123}", name="Speakers", is_default=True
        )
...
class TestAudioSwitchAction:
    ...
    def test_only_triggers_on_note_on(self):
        ...
        with patch(
            "k2deck.actions.audio_switch.set_default_audio_device_by_name"
        ) as mock:
            event = MidiEvent(
    ...
    def test_multiple_devices_cycles(self, mock_cycle):
        ...
        action = AudioSwitchAction(
            {"devices": ["Speakers", "Headphones"], "type": "output"}
        )
        event = MidiEvent(
            type="note_on", channel=16, note=36, cc=None, value=127, timestamp=0.0
        )
